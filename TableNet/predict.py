...
class Predict:
    ...
    def predict(self, image: Image) -> List[pd.DataFrame]:
        ...
        plt.imshow(torch.squeeze(table_mask))
        ...
        return tables, table_mask, column_mask
...
@click.option('--model_weights', default="./data/best_model_resnet.ckpt")
def predict(image_path: str, model_weights: str) -> List[pd.DataFrame]:
    ...
    tables, table_mask, column_mask = pred.predict(image)
    for i, out in enumerate(tables):
        path = './results/img4/'+str(i)+'.csv'
        out.to_csv(path, index=False)
    print(pred.predict(image))

    plt.imshow(table_mask)
    plt.savefig('./results/img4/table_mask.jpg')
    plt.imshow(column_mask)
    plt.savefig('./results/img4/column_mask.jpg')
